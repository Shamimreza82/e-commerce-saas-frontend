...
import { useCategories, useCreateCategory, useUpdateCategory, useDeleteCategory } from "@/modules/category/category.hooks";
...
import { Trash2, Edit, Layers, Save, X } from "lucide-react";
  ...
  return (
    <div className="space-y-10">
      <div className="flex flex-col gap-2">
        ...
        <p className="text-zinc-500 text-lg">Organize your products into logical groups.</p>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-10">
        ...
        <div className="lg:col-span-1">
          <Card className="border-none shadow-xl ring-1 ring-zinc-200 dark:ring-zinc-800 sticky top-28">
            <CardHeader>
              <CardTitle className="text-xl font-bold">{editingId ? "Edit Category" : "Create Category"}</CardTitle>
              <CardDescription>Enter the name of your category.</CardDescription>
            </CardHeader>
            <CardContent>
              <form onSubmit={handleSubmit(onSubmit)} className="space-y-6">
                <div className="space-y-2">
                  <Label htmlFor="name" className="font-bold uppercase text-[10px] tracking-widest text-zinc-400">Category Name</Label>
                  <Input 
                    id="name" 
                ...
                <div className="flex gap-3">
                  <Button type="submit" className="flex-1 h-12 rounded-xl font-bold" disabled={isCreating || isUpdating}>
                    {editingId ? <><Save className="mr-2 h-4 w-4" /> Update</> : <><Layers className="mr-2 h-4 w-4" /> Create</>}
                  </Button>
                  {editingId && (
                    <Button 
                      type="button" 
            ...
            <CardContent className="p-0">
              {isLoading ? (
                ...
                <div className="text-center py-20">
                  ...
                  <p className="text-zinc-500 font-medium">No categories found yet.</p>
                </div>
              ) : (
                <div className="divide-y divide-zinc-50 dark:divide-zinc-900">
                  {categories.map((cat: any) => (
                    <div key={cat.id} className="flex items-center justify-between p-6 hover:bg-zinc-50/50 transition-colors group dark:hover:bg-zinc-900/30">
                      <div className="flex items-center gap-4">
                        ...
                        <div>
                          <p className="font-bold text-lg text-zinc-950 dark:text-zinc-50">{cat.name}</p>
                          <p className="text-xs font-bold uppercase tracking-widest text-zinc-400">{cat._count?.products || 0} Products</p>
                        </div>
                      </div>
